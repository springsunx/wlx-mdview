...
static void build_css(StrBuf* sb) {
    ...
    ".mdv-cb{position:absolute;top:6px;right:6px;padding:3px 8px;font:11px/1 'Segoe UI',sans-serif;"
    "color:#57606a;background:#e8eaed;border:1px solid #d0d7de;border-radius:4px;cursor:pointer;"
    "opacity:0;-ms-transition:opacity .15s;transition:opacity .15s;z-index:1;-ms-user-select:none;user-select:none}"
    "pre:hover .mdv-cb{opacity:1}.mdv-cb:hover{background:#d0d7de;color:#24292e}"
    ".mdv-cb.on{color:#28a745;pointer-events:none}"
    "body.dark .mdv-cb{color:#8b949e;background:#30363d;border-color:#484f58}"
    "body.dark .mdv-cb:hover{background:#484f58;color:#c9d1d9}"
    "body.dark .mdv-cb.on{color:#56d364}"
...
static void build_js(StrBuf* sb) {
    ...
    "function mdvInitCopyButtons(){var pres=document.getElementsByTagName('pre');"
    "for(var i=0;i<pres.length;i++){(function(pre){"
    "if(pre.getAttribute('data-mdv-mermaid'))return;"
    "var btn=document.createElement('span');btn.className='mdv-cb';btn.innerText='Copy';"
    "btn.onclick=function(){"
    "var code=pre.getElementsByTagName('code');"
    "var txt=code.length?(code[0].textContent||code[0].innerText):(pre.textContent||pre.innerText);"
    "txt=txt.replace(/\\r\\n/g,'\\n').replace(/\\r/g,'\\n');"
    ...
    "btn.className='mdv-cb on';btn.innerText='Copied!';"
    "setTimeout(function(){btn.className='mdv-cb';btn.innerText='Copy';},1500);"
    "};pre.appendChild(btn);"
    "})(pres[i]);}}"
